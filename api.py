...
# إخفاء تحذيرات SSL لكي لا ينكشف البوت
...
def fetch_product_native(session, store_url):
    """محرك سحب المنتجات باستخدام requests (بدون حظر فوري)"""
    ...
    # مسار 1
    try:
        r = session.get(f"{store_url}/products.json?limit=250", timeout=10, verify=False)
    ...
    # مسار 2
    try:
        r = session.get(f"{store_url}/collections/all/products.json?limit=250", timeout=10, verify=False)
        if r.status_code == 200:
            vid, p = extract_vid(r.json())
            if vid: return vid, p
    except: pass
...
def api_endpoint(cc: str = Query(...), url: str = Query(...), proxy: str = Query(None)):
    try:
        ...
        # استخدام requests العادية تماماً
        with requests.Session() as session:
            if proxies: session.proxies.update(proxies)
            session.headers.update(headers)

            # 1. سحب المنتج
            variant_id, price = fetch_product_native(session, store_url)
            if not variant_id: return JSONResponse(content=safe_response("Store Blocked Product Fetching", "-"))
            ...
            # 3. محاكاة الانتقال لصفحة الدفع كزائر (POST Request لتوليد التوكن)
            ...
            if "/cart" in final_url and "/checkout" not in final_url:
                return JSONResponse(content=safe_response("Item Removed / Cart Empty", price))
            ...
            if res_pci.status_code != 200: return JSONResponse(content=safe_response("Stripe PCI Blocked IP", price))
